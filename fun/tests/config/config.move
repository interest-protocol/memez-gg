...
fun set_up_treasury_wrong_decimals() {
    ...
    usdc::init_for_testing(world.scenario.ctx());

    world.scenario.next_epoch(ADMIN);

    let usdc_metadata = world.scenario.take_shared<CoinMetadata<USDC>>();
    let usdc_treasury_cap = world.scenario.take_from_sender<TreasuryCap<USDC>>();
...
fun start(): World {
    ...
    let metadata = scenario.take_shared<CoinMetadata<GG>>();
