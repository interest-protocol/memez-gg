...
fun test_new_coin() {
    ...
    let stable_config = world.stable_config;
    ...
    assert_eq(
        ...
        stable_config.liquidity_provision(total_supply),
    );
    ...
    assert_eq(fr.meme_sale_amount(), stable_config.meme_sale_amount(total_supply));
    assert_eq(fr.quote_balance().value(), 0);
    assert_eq(fr.meme_balance().value(), stable_config.meme_sale_amount(total_supply));

    assert_eq(
        memez_stable::meme_reserve(&mut memez_fun).value(),
        total_supply - dev_allocation - stable_config.meme_sale_amount(total_supply) - stable_config.liquidity_provision(total_supply),
    );
...
fun test_coin_end_to_end() {
    ...
    let mut fr = memez_fixed_rate::new<Meme, SUI>(
        target_sui_liquidity,
        balance::create_for_testing(stable_config.meme_sale_amount(total_supply)),
...
fun test_token_end_to_end() {
    ...
    assert_eq(meme_token.value(), stable_config.meme_sale_amount(total_supply));
    ...
    assert_eq(meme_coin.burn_for_testing(), stable_config.meme_sale_amount(total_supply));
...
fun test_distribute_stake_holders_allocation() {
    ...
    let mut fr = memez_fixed_rate::new<Meme, SUI>(
        target_sui_liquidity,
        balance::create_for_testing(stable_config.liquidity_provision(total_supply)),
        world.config.fees<DefaultKey>().swap(vector[STAKE_HOLDER]),
    );
...
fun start(): World {
    ...
    let stable_config = memez_stable_config::new(vector[
        MAX_TARGET_SUI_LIQUIDITY,
        LIQUIDITY_PROVISION,
        MEME_SALE_AMOUNT,
    ]);

    World { config, migrator_list, scenario, stable_config }
}
