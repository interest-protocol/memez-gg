...
fun test_new() {
    let fees = memez_fees::new(
        vector[
            ...
            vector[2_500, 2_500, 4_000, 1_000, 200 * POW_9],
            vector[3_500, 3_500, 3_000, 200],
        ],
        vector[
            vector[INTEGRATOR, INTEREST],
            vector[STAKE_HOLDER_1, STAKE_HOLDER_2, INTEGRATOR],
            vector[STAKE_HOLDER_1, STAKE_HOLDER_2, INTEGRATOR, INTEREST],
            vector[STAKE_HOLDER_1, STAKE_HOLDER_2, INTEGRATOR],
        ],
    );
    ...
    assert_eq!(payloads[1].payload_percentages(), vector[2_500, 2_500, 5_000]);
    assert_eq!(payloads[2].payload_percentages(), vector[2_500, 2_500, 4_000, 1_000]);
    assert_eq!(payloads[3].payload_percentages(), vector[3_500, 3_500, 3_000]);

    assert_eq!(payloads[0].payload_recipients(), vector[INTEGRATOR, INTEREST]);
    assert_eq!(payloads[1].payload_recipients(), vector[STAKE_HOLDER_1, STAKE_HOLDER_2, INTEGRATOR]);
    assert_eq!(payloads[2].payload_recipients(), vector[STAKE_HOLDER_1, STAKE_HOLDER_2, INTEGRATOR, INTEREST]);
    assert_eq!(payloads[3].payload_recipients(), vector[STAKE_HOLDER_1, STAKE_HOLDER_2, INTEGRATOR]);
}
...
//     let swap_fee = fees.swap(vector[@0x7]);
//     let migration_fee = fees.migration(vector[@0x8]);
...
//         .recipients()[1]
//         .recipient_data();
//     let (last_swap_recipient_addy, last_swap_recipient_bps) = swap_fee
//         .recipients()[1]
//         .recipient_data();
//     let (last_migration_recipient_addy, last_migration_recipient_bps) = migration_fee
//         .recipients()[1]
//         .recipient_data();
