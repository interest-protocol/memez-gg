...
fun test_buy_with_deadline() {
    ...
    dapp.tx!(|_, clock, otc, scenario| {
        let (amount_in, fee_in) = otc.deadline.get_amount_in(SALE_AMOUNT);
        ...
        assert_eq(otc.deadline.balance(), 0);
    });

    dapp.end();
}

#[test, expected_failure(abort_code = errors::EHasNoDeadline, location = memez_otc)]
fun test_buy_with_deadline_error_has_no_deadline() {
    let mut dapp = deploy();
...
#[test, expected_failure(abort_code = errors::EVestedOTC, location = memez_otc)]
fun test_buy_with_deadline_error_vested_otc() {
    let mut dapp = deploy();
...
#[test, expected_failure(abort_code = errors::EDeadlinePassed, location = memez_otc)]
fun test_buy_with_deadline_error_deadline_passed() {
    let mut dapp = deploy();
...
#[test]
fun test_buy_with_vesting() {
    let mut dapp = deploy();

    dapp.tx!(|_, clock, otc, scenario| {
        let (amount_in, fee_in) = otc.vesting.get_amount_in(SALE_AMOUNT);
        ...
        assert_eq(meme_coin_vesting.start(), 100);
        assert_eq(meme_coin_vesting.duration(), VESTING_PERIOD);

        destroy(meme_coin_vesting);

        assert_eq(otc.vesting.balance(), 0);
    });
...
#[test, expected_failure(abort_code = errors::EHasDeadline, location = memez_otc)]
fun test_buy_with_vesting_error_has_deadline() {
    let mut dapp = deploy();
...
#[test, expected_failure(abort_code = errors::ENormalOTC, location = memez_otc)]
fun test_buy_with_vesting_error_normal_otc() {
    let mut dapp = deploy();
...
#[test]
fun test_buy_with_vesting_and_deadline() {
    let mut dapp = deploy();

    dapp.tx!(|_, clock, otc, scenario| {
        let (amount_in, fee_in) = otc.deadline_vesting.get_amount_in(SALE_AMOUNT);
        ...
        assert_eq(excess_sui.burn_for_testing(), 0);
        assert_eq(meme_coin_vesting.balance(), SALE_AMOUNT);
        assert_eq(meme_coin_vesting.start(), DEADLINE - 1);
        assert_eq(meme_coin_vesting.duration(), VESTING_PERIOD);

        destroy(meme_coin_vesting);

        assert_eq(otc.deadline_vesting.balance(), 0);
    });
...
#[test, expected_failure(abort_code = errors::EDeadlinePassed, location = memez_otc)]
fun test_buy_with_vesting_and_deadline_error_deadline_passed() {
    let mut dapp = deploy();
...
#[test, expected_failure(abort_code = errors::ENormalOTC, location = memez_otc)]
fun test_buy_with_vesting_and_deadline_error_normal_otc() {
    let mut dapp = deploy();
...
#[test, expected_failure(abort_code = errors::EHasNoDeadline, location = memez_otc)]
fun test_buy_with_vesting_and_deadline_error_has_no_deadline() {
    let mut dapp = deploy();
